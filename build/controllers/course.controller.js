...
const course_servise_1 = require("../servisces/course.servise");
...
exports.uploadCourse = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        (0, course_servise_1.createCourse)(data, res, next);
    }
    catch (error) {
...
exports.addAnswer = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        const newAnswer = {
            ...
            opdatedAt: new Date().toISOString(),
        };
        // add this answer to question
        question.questionReplies?.push(newAnswer);
        await course?.save();
        if (req.user?._id === question.user._id) {
            // create notifoccation about answer
            await notification_model_1.default.create({
                user: req.user?._id,
...
exports.addReview = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        const userCourseList = req.user?.courses;
        const courseId = req.params.id;
        // check if courseId alrady exist in user course list on _id
        ...
        if (course) {
            course.ratings = avg / course.reviews.length; // example 2reviews , 4 and 5.   4+5=9,  9 / 2 = 4.5
        }
        await course?.save();
        await redis_1.redis.set(courseId, JSON.stringify(course), "EX", 604800); //7days
        // create notification
...
// get all courses fro admin
exports.getAllCoursesAdmin = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        (0, course_servise_1.getAllCoursesService)(res);
    }
    catch (error) {
        return next(new ErrorHandler_1.default(error.message, 500));
    }
});
// delete course anly admin can
